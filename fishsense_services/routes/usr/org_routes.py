...
async def create_organization_route(request: OrgRequest):
    
    data = request.model_dump()
    
    try:
        
        # valid = jwt.decode(data["token"], JWT_SECRET, algorithms=[JWT_ALGORITHM])
        ...
        success = False
        with Database() as db:
            success = db.exec_script("fishsense-database/fishsense_database/scripts/insert_scripts/create_org.sql", HTTP_CODES.POST.value, data) 
        ...
        return JSONResponse(
            status_code=201,
            content=success # should be json with information
        )
            
    except ValueError as e:
        return JSONResponse(
            status_code=400,
            content={"Error": str(e)}  # Send the error message as string
        )
